...
def preprocess_batch(batch):
    if isinstance(batch, str):
        # if a file path is passed in, load the image and convert it to a tensor
        image = Image.open(batch)
        tensor = transforms.functional.to_tensor(image)
        tensor = tensor.unsqueeze(0)
    else:
        # if a BytesIO object is passed in, convert it to a tensor directly
        tensor = transforms.functional.to_tensor(Image.open(batch))
        tensor = tensor.unsqueeze(0)
...
def predict():
    ...
    # preprocess images
    content_tensor = preprocess_batch(content_image)
    style_tensor = preprocess_batch(style_image)

    # run style transfer
    style_v = Variable(style_tensor)
    content_v = Variable(content_tensor)
    style_model.setTarget(style_v)
    output = style_model(content_v)
    output_path = os.path.join(app.config['STATIC_FOLDER'], 'output.jpg')
    ...
    # return prediction
    return render_template('index.html', prediction='output.jpg')
